...
#define CATCH_INTERNAL_SUPPRESS_GLOBALS_WARNINGS                                                                                                               \
  _Pragma("clang diagnostic push") _Pragma("clang diagnostic ignored \"-Wexit-time-destructors\"") _Pragma("clang diagnostic ignored \"-Wglobal-constructors\"")
...
namespace Catch {
  ...
  public: // construction/ assignment
    ...
    StringRef(StringRef &&other) noexcept : m_start(other.m_start), m_size(other.m_size), m_data(other.m_data) { other.m_data = nullptr; }
...
inline auto operator"" _catch_sr(char const *rawChars, std::size_t size) noexcept -> Catch::StringRef { return Catch::StringRef(rawChars, size); }
...
#define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE2(TestName, TestFuncName, Name, Tags, TmplTypes, TypesList)                                                   \
  ...
  namespace {                                                                                                                                                  \
    ...
    static int INTERNAL_CATCH_UNIQUE_NAME(globalRegistrar) = []() {                                                                                            \
      using TestInit =                                                                                                                                         \
          Catch::combine<INTERNAL_CATCH_REMOVE_PARENS(TmplTypes)>::with_types<INTERNAL_CATCH_MAKE_TYPE_LISTS_FROM_TYPES(TypesList)>::into<TestName>::type;     \
      TestInit();                                                                                                                                              \
      return 0;                                                                                                                                                \
    }();                                                                                                                                                       \
  }                                                                                                                                                            \
  ...
  namespace {                                                                                                                                                  \
    ...
    struct TestNameClass {                                                                                                                                     \
      ...
      TestNameClass(Ts... names) {                                                                                                                             \
        CATCH_INTERNAL_CHECK_UNIQUE_TYPES(CATCH_REC_LIST(INTERNAL_CATCH_REMOVE_PARENS, __VA_ARGS__))                                                           \
        using expander = int[];                                                                                                                                \
        (void)expander{(Catch::AutoReg(Catch::makeTestInvoker(&TestName<Types>::test), CATCH_INTERNAL_LINEINFO, #ClassName, Catch::NameAndTags{names, Tags}),  \
                        0)...}; /* NOLINT */                                                                                                                   \
      }                                                                                                                                                        \
    };                                                                                                                                                         \
...
#define INTERNAL_CATCH_INFO(macroName, log)                                                                                                                    \
  Catch::ScopedMessage INTERNAL_CATCH_UNIQUE_NAME(scopedMessage)(Catch::MessageBuilder(macroName##_catch_sr, CATCH_INTERNAL_LINEINFO, Catch::ResultWas::Info)  \
                                                                 << log);
...
namespace Catch {
  namespace Generators {
    ...
    GeneratorWrapper<T> map(Func &&function, GeneratorWrapper<T> &&generator) {
      return GeneratorWrapper<T>(pf::make_unique<MapGenerator<T, T, Func>>(std::forward<Func>(function), std::move(generator)));
    }
...
namespace Catch {
  namespace Generators {
    ...
    public:
      RandomFloatingGenerator(Float a, Float b) : m_rand(getCurrentContext().getConfig()->rngSeed()), m_dist(a, b) { static_cast<void>(next()); }
    ...
    public:
      RandomIntegerGenerator(Integer a, Integer b) : m_rand(getCurrentContext().getConfig()->rngSeed()), m_dist(a, b) { static_cast<void>(next()); }
    ...
    namespace Impl {
      namespace NSStringMatchers {
        ...
        struct Contains : StringHolder {
          ...
          bool match(NSString *str) const { return (str != nil || m_substr == nil) && [str rangeOfString:m_substr].location != NSNotFound; }
...
namespace Catch {
  ...
  class ReporterRegistrar {
    ...
    class ReporterFactory : public IReporterFactory {
      ...
      virtual IStreamingReporterPtr create(ReporterConfig const &config) const override { return std::unique_ptr<T>(new T(config)); }
...
namespace Catch {
  ...
  auto BenchmarkLooper::getResolution() -> uint64_t { return getEstimatedClockResolution() * getCurrentContext().getConfig()->benchmarkResolutionMultiple(); }
...
namespace Catch {
  ...
  namespace Generators {
    ...
    auto acquireGeneratorTracker(SourceLineInfo const &lineInfo) -> IGeneratorTracker & { return getResultCapture().acquireGeneratorTracker(lineInfo); }
...
namespace Catch {
  namespace TestCaseTracking {
    ...
    SectionTracker::SectionTracker(NameAndLocation const &nameAndLocation, TrackerContext &ctx, ITracker *parent) : TrackerBase(nameAndLocation, ctx, parent) {
      if (parent) {
...
namespace Catch {
  ...
  bool TestSpec::NamePattern::matches(TestCaseInfo const &testCase) const { return m_wildcardPattern.matches(toLower(testCase.name)); }
...
namespace Catch {
  ...
  ConsoleReporter::ConsoleReporter(ReporterConfig const &config)
      : StreamingReporterBase(config), m_tablePrinter(new TablePrinter(config.stream(), {{"benchmark name", CATCH_CONFIG_CONSOLE_WIDTH - 32, ColumnInfo::Left},
